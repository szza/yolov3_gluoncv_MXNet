...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, args):
    ...
    val_loader = gluon.data.DataLoader(
        val_dataset.transform(YOLO3DefaultValTransform(width, height)),  # validation时，是以某个固定尺寸的(416, 416)
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    """ 
        为了提高mAP，使用了一些训练技巧，
        可以参考论文《Bag of Tricks for Image Classification with Convolutional Neural Networks》
    """
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        btic = time.time()
        mx.nd.waitall()
        net.hybridize()  # 为u什么每次都要初始化一次

        for i, batch in enumerate(train_data):
            batch_size = batch[0].shape[0]
            """" split_and_load 就是把一个batch的数据分到几个gpu上面，但是对于只一个GPU的，这个函数可有可无，此时 
                    data = gluon.utils.split_and_load(batch[0], ctx_list=ctx, batch_axis=0) 等价于data = batch[0] 
                下面的fixed_targets, gt_boxes类似
            """
