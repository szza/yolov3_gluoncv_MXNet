...
class YOLOV3Loss(Loss):
    ...
    def __init__(self, batch_axis=0, weight=None, **kwargs):
        ...
        '''
            对分类使用的是二分类的logistics交叉熵损失函数
        '''
        self._sigmoid_ce = gluon.loss.SigmoidBinaryCrossEntropyLoss(from_sigmoid=False)  # 置信度损失和分类损失
    ...
    def hybrid_forward(self, F, objness, box_centers, box_scales, cls_preds,
        ...
        # all_targets: shape = [
            [batch, h*W*9, 1] ,  objness_t        : 0 for negative 1 for positive, -1 for ignore.   
            [batch, h*W*9, 2] ,  center_targets
            [batch, h*W*9, 2] ,  scale_targets
            [batch, h*W*9, 2] ,  weight_t
            [batch, h*W*9, 20],  class_targets
            [batch, h*W*9, 20],  class_mask
            ]
            
            # denorm = objness_t.shape[1:].prod().astype('float32') --> h*w*9
                    shape_array 返回的是一个1D ndarray数组，值是传入的数组objness_t的维度信息
                    denorm = h*W*9
        ...
        '''
        损失函数：
            objness,center, class_pred 都是用的sigmoid
            scale用的是L1

        hard_objness_t 即 objness_t
        new_objness_mask: 非正样本都设置为0，其余都是1

        self._sigmoid_ce(objness, hard_objness_t, new_objness_mask),
            objness:是预测输出
            hard_objness_t:是ground-truth 
            new_objness_mask: 选择忽略哪些grid cell，即在计算损失的时候不考虑哪些grid cell。
                损失函数内部：objness, hard_objness_t,计算出损失loss后，再 loss * new_objness_mask
